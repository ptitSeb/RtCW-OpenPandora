...
#ifdef PANDORA
void IN_UpDown( void ) {kb[KB_DOWN].active = 0; IN_KeyDown( &kb[KB_UP] );}
#else
void IN_UpDown( void ) {IN_KeyDown( &kb[KB_UP] );}
#endif
void IN_UpUp( void ) {IN_KeyUp( &kb[KB_UP] );}
#ifdef PANDORA
void IN_DownDown( void ) {IN_ToggleKeyDown( &kb[KB_DOWN] );}
void IN_DownUp( void ) {IN_ToggleKeyUp( &kb[KB_DOWN] );}
#else
...
void CL_MouseMove( usercmd_t *cmd ) {
	...
	int cg_crossHairStatus = 0/*(serverCursorHint==HINT_PLYR_ENEMY)?1:0*/;
	
		g_lastFireTime = cl.cgameLastFireTime;
		cg_crossHairStatus = cl.cgameCrossHairStatus;
...
#ifdef AUTOAIM
	const float m_hoverSensitivity = 0.2f;
	if (cg_crossHairStatus != 0)
	{
...
//				normAngle += 10;	// Removed by BTO, 2003/05/14, I hate it
	...
	if ( kb[KB_STRAFE].active ) {
		cmd->rightmove = ClampChar( cmd->rightmove + m_side->value * mx * cl_pitchSensitivity);
	} else {
		cl.viewangles[YAW] -= m_yaw->value * mx * cl_pitchSensitivity;
	}
