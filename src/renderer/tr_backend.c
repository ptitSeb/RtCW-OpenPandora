...
#define ZOMBIEFX_FADEOUT_TIME_SEC       ( 0.001f * ZOMBIEFX_FADEOUT_TIME )
...
void RE_StretchRaw( int x, int y, int w, int h, int cols, int rows, const byte *data, int client, qboolean dirty ) {
	...
	if ( cols != tr.scratchImage[client]->width || rows != tr.scratchImage[client]->height /*dirty*/ ) {
		tr.scratchImage[client]->width = tr.scratchImage[client]->uploadWidth = cols;
		tr.scratchImage[client]->height = tr.scratchImage[client]->uploadHeight = rows;
		#ifdef HAVE_GLES
//ri.Printf( PRINT_ALL, "texnum=%i, premier point %x )\n", tr.scratchImage[client]->texnum, *(unsigned int*)data );
