...
vidmode_t r_vidModes[] =
{
	...
	{ "Mode 13: 1920x1200 (wide)",1920,  1200,   1 }     //----(SA)	added
};
...
void R_TakeScreenshot( int x, int y, int width, int height, char *fileName ) {
	...
	byte *source = ri.Hunk_AllocateTempMemory( p2width * p2height * 4 );
...
void R_LevelShot( void ) {
	...
	while (p2width<glConfig.vidWidth) p2width*=2;
	while (p2height<glConfig.vidHeight) p2height*=2;
	source = ri.Hunk_AllocateTempMemory( p2width * p2height * 4 );
	#else
